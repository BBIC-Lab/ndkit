...
class Model(nn.Module):
    def __init__(self, cfg):
        super().__init__()
        input_dim = getattr(cfg, "input_size")
        output_dim = getattr(cfg, "output_size")
        seq_len = getattr(cfg, "seq_len")
        head_size = getattr(cfg, "head_size")
        ...
        baseconfig = GPTConfig(hidden_size, seq_len, model_type='RWKV',
                               rwkv_emb_scale=rwkv_emb_scale, rwkv_tiny_attn=rwkv_tiny_attn,
                               rwkv_tiny_head=rwkv_tiny_head,
                               n_layer=num_layers, n_head=num_heads, n_embd=hidden_size,
                               n_attn=hidden_size, n_ffn=hidden_size)
        self.blocks = nn.Sequential(*[Block(baseconfig, i) for i in range(num_layers)])
    ...
    def forward(self, x):
        # [bs, seq_len, n_dim]
        x = self.embed(x) # [bs, seq_len, n_dim] -> [bs, seq_len, hidden_size]
        x = self.blocks(x)
        hn = x.mean(dim=1)
        x = self.MLP(self.drop(hn))
        return x
...
class RWKV_TimeMix(nn.Module):
    ...
    def forward(self, x):
        B, T, C = x.size()
        TT = self.ctx_len
        w = F.pad(self.time_w, (0, TT)) # self.time_w是一个[n_head, ctx_len]的矩阵，不同头有不同的衰减速度
        w = torch.tile(w, [TT])
